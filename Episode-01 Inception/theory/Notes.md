...
**`async:`**
Downloads the script asynchronously in the background. 
Executes the script as soon as it's downloaded, potentially before the HTML parsing is complete. 
Execution order is not guaranteed and can be unpredictable. 

**`defer:`**
Downloads the script asynchronously in the background. 
Executes the script only after the HTML parsing is complete, but before the DOMContentLoaded event. 
